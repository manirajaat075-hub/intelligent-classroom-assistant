...
const DashboardLayout = ({ role, onLogout, activeTab, setActiveTab, children, classroomId }) => {
  ...
  return (
    <div className="flex min-h-screen text-slate-200">
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900/50 backdrop-blur-xl border-r border-slate-800 transition-transform duration-300 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
